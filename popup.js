...
document.getElementById('runSuiteScript').addEventListener('click', () => {
    const userScript = document.getElementById('suiteScriptInput').value;

    if (userScript.trim()) {
        // Send the user's script to the content script
        chrome.tabs.query({ active: true, currentWindow: true }, (tabs) => {
            chrome.tabs.sendMessage(tabs[0].id, { action: 'runCustomScript', script: userScript });
        });
    } else {
  ...
  chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
    if (message.type === 'OPEN_RESULTS_TAB') {
      ...
      chrome.tabs.create({ url: chrome.runtime.getURL('hresults.html') }, function(tab) {
        // Passing the hierarchy data to the new tab once it's fully loaded
        chrome.tabs.onUpdated.addListener(function listener(tabId, changeInfo) {
          if (tabId === tab.id && changeInfo.status === 'complete') {
            chrome.tabs.sendMessage(tabId, { type: 'DISPLAY_HIERARCHY', hierarchy: message.hierarchy });
            chrome.tabs.onUpdated.removeListener(listener);  // Remove
          }
        });
      });
    }
...
if (message.type === 'QUERY_RESULTS') {
    ...
    hideLoader();
} else if (message.type === 'DISPLAY_RESULT') {
    alert('Script result: ' + "Results in the next Popup"); 
